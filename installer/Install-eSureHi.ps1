...
if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # The SFX wrapper removes its temporary extraction folder once this
    # process exits. Wait for the elevated process so it can still read the
    # installer script and payload from that folder.
    $elevatedProcess = Start-Process -FilePath 'powershell.exe' -ArgumentList $elevatedArgs -Verb RunAs -PassThru -Wait
